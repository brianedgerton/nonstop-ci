...
module.exports = function( organization, repository ) {
	var Repository = machina.Fsm.extend( {
		...
		states: {
			'fetching-branches': {
				_onEnter: function() {
					var topic = [ organization, repository.name, '#' ].join( '.' );
				...
				_onEnter: function() {
					...
					if ( !this.tree ) {
						if ( this.mainBranch && this.mainBranch.commit.sha ) {
							this.tree = tree( this.organization, this.repository.name, this.mainBranch.commit.sha );
						} else {
